...
export default function InterviewPage() {
  ...
  useEffect(() => {
    ...
    const initSession = async () => {
      try {
        // Fetch the session to get the question that was already selected with user's filters
        ...
        setCode(data.question.starter_code || "def solution():\n    pass");
        setLoading(false);
      } catch (err) {
  ...
  const handleCodeChange = (newCode: string) => {
    setCode(newCode);
    wsRef.current?.send("CODE_SNAPSHOT", { language: "python", code: newCode, cursor: { line: 0, col: 0 } });
  };
